...
async def handler(interaction: discord.Interaction, allgemein_channel: discord.TextChannel = None, spam_channel: discord.TextChannel = None, keyword: str = None, gif_channel: discord.TextChannel = None, log_channel: discord.TextChannel = None, musiccommand_channel: discord.TextChannel = None, temp_template_channel: discord.VoiceChannel = None, temp_category: discord.CategoryChannel = None, botcommand_channel: discord.TextChannel = None, adminrole: discord.Role = None, botrole: discord.Role = None, picture_channel: discord.TextChannel = None, api_key_weather: str = None, base_url: str = None, geonames_username: str = None):
    ...
    if all(variable is None for variable in variables):
        ...
        embed.add_field(name="Allgemeiner Channel", value=f"{data.get('ALLGEMEIN_ID')}", inline=False)
        embed.add_field(name="Spam Channel", value=f"{data.get('SPAM_CHANNEL_ID')}", inline=False)
        embed.add_field(name="Spam Schlüsselwort", value=f"{data.get('SPAM_KEYWORD')}", inline=False)
        embed.add_field(name="Gif Channel", value=f"{data.get('GIF_ID')}", inline=False)
        embed.add_field(name="Bilderchannel", value=f"{data.get('PICTURE_CHANNEL_ID')}", inline=False)
        embed.add_field(name="Logchannel", value=f"{data.get('LOG_CHANNEL_ID')}", inline=False)
        embed.add_field(name="Musikbefehlchannel", value=f"{data.get('MUSIC_CHANNEL_ID')}", inline=False)
        embed.add_field(name="Temp-Vorlage-Channel", value=f"{data.get('TEMP_CHANNEL_ID')}", inline=False)
        embed.add_field(name="Temp-Kategorie", value=f"{data.get('TEMP_CATEGORY_ID')}", inline=False)
        embed.add_field(name="Botbefehlechannel", value=f"{data.get('BOT_CHANNEL_ID')}", inline=False)
